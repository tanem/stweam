...
/**
 * Generate a `result` object from the incoming tweet object
 * based on the `_receive` property. If `_receive` is empty
 * then the entire tweet object is pushed.
 *
...
Stweam.prototype._transform = function(tweet, encoding, done){
  if (this._receive.length === 0) {
    this.push(tweet);
  } else {
    var result = {};
    this._receive.forEach(function(key){
...
Stweam.prototype.receive = function(receive){
