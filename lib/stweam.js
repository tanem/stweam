...
Stweam.prototype = Object.create(events.EventEmitter.prototype);
...
Stweam.prototype._initBackoffs = function(){
  ...
  this.http420ErrorBackoff = backoff.exponential(60 * 1000);
};
...
Stweam.prototype._start = function(){
  ...
  if (this.parserStream) this.parserStream.unpipe(this.publisher);
  ...
  this.request.on('error', function(){
    this.emit('request error ... reconnecting');
    co(function*(){
  ...
  this.request.on('response', function(response){
    ...
    if (code === 420 || code >= 500) {
      this.emit('rate limited response ... reconnecting');
      co(function*(){
        yield this.http420ErrorBackoff;
        this._start();
      }).call(this);
      return;
    }
    ...
    if (code > 200) {
      this.emit('http error response ... reconnecting');
      co(function*(){
    ...
    // `this.publisher` manually however.
    this.parserStream = response.pipe(this.parser);
    this.parserStream.pipe(this.publisher);
